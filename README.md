...
graph TD
    ...
    B -->|Зчитування статичних слайдерів| E[db.json]
```
...
- Логіка роботи: Завантажує каталог букетів через клієнт apiClient (Axios).
- Зв'язок з бекендом: 
  - Локально: Запити проксуються через конфігурацію vite.config.js з порту 4000 на порт 3001 бекенду (правила проксі для /api та /photos).
...
- Логіка роботи: Надає REST API для роботи з колекцією букетів, бестселерів та коментарів.
